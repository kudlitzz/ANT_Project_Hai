...
{
    class ANT_HAI_RPT_ShipmentList : IReportInfo_Version_1_1
    {
        ...
        {  
            ...
            if (FirstSelectID != "")
            {
                sqlText = "select r.* from LV_Order t with(nolock) cross apply [ANT_F_HAI_RPT_ShipmentList](t.ord_ID) r where t.ord_ID in (" + SelectID + ") order by r.ord_ID";
